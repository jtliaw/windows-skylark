...
class PackageManager:
    """
    管理Argos Translate语言包的安装、卸载和存储，专门优化Windows环境。
    """
    def __init__(self, status_queue):
        self.status_queue = status_queue
        self.package_index = []
    ...
    def _setup_windows_ssl(self):
        """Windows SSL配置"""
        try:
            os.environ['SSL_CERT_FILE'] = certifi.where()
            os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
            os.environ['CURL_CA_BUNDLE'] = certifi.where()
            self.status_queue.put("Windows SSL配置已设置")
        except Exception as e:
            self.status_queue.put(f"SSL配置警告: {e}")
    ...
    def _get_package_dir(self):
        ...
        try:
            ...
            self.status_queue.put(f"使用包目录: {custom_package_dir}")
            return str(custom_package_dir)
        except Exception as e:
            # 备用目录
            fallback_dir = Path.home() / "argos_packages"
            fallback_dir.mkdir(parents=True, exist_ok=True)
            self.status_queue.put(f"使用备用目录: {fallback_dir}")
    ...
    def _is_valid_cache(self, cached_index):
        """验证缓存是否有效"""
        return (isinstance(cached_index, list) and 
                len(cached_index) > 0 and 
                len(cached_index) <= 100)
    ...
    def generate_official_language_packages(self):
        ...
        # 语言包大小估计
        PACKAGE_SIZE_ESTIMATES = {
            'en': 150, 'es': 140, 'fr': 145, 'de': 160, 'it': 135,
            'pt': 130, 'ru': 180, 'zh': 200, 'ja': 190, 'ko': 170,
            'ar': 160, 'hi': 150, 'default': 140
        }
        
        # 支持的语言列表
        SUPPORTED_LANGUAGES = [
            ('en', '英语'), ('es', '西班牙语'), ('fr', '法语'), ('de', '德语'),
            ('it', '意大利语'), ('pt', '葡萄牙语'), ('ru', '俄语'), ('zh', '中文'),
            ('ja', '日语'), ('ko', '韩语'), ('ar', '阿拉伯语'), ('az', '阿塞拜疆语'),
            ('ca', '加泰罗尼亚语'), ('cs', '捷克语'), ('da', '丹麦语'), ('nl', '荷兰语'),
            ('eo', '世界语'), ('fi', '芬兰语'), ('el', '希腊语'), ('he', '希伯来语'),
            ('hi', '印地语'), ('hu', '匈牙利语'), ('id', '印尼语'), ('ga', '爱尔兰语'),
            ('ms', '马来语'), ('fa', '波斯语'), ('pl', '波兰语'), ('sk', '斯洛伐克语'),
            ('sv', '瑞典语'), ('tr', '土耳其语'), ('uk', '乌克兰语')
        ]
        ...
        for package_code in official_packages:
            ...
            from_name = lang_dict.get(from_code, from_code)
            to_name = lang_dict.get(to_code, to_code)
            
            size = (PACKAGE_SIZE_ESTIMATES.get(from_code, 140) + 
                   PACKAGE_SIZE_ESTIMATES.get(to_code, 140))
    ...
    def is_package_installed(self, from_code, to_code):
        ...
        try:
            ...
            self.status_queue.put(f"[调试] API检查失败: {e}")
    ...
    def is_package_available(self, from_code, to_code):
        ...
        return package_code in self.get_official_packages()

    def _install_via_python_api_windows(self, from_code, to_code, progress_callback=None):
        """Windows优化版Python API安装"""
        import requests
        
        def log(message):
            self.status_queue.put(f"[Windows API] {message}")
            print(f"[Windows API] {message}")
        
        max_retries = 3
        retry_delays = [3, 7, 15]  # 递增延迟
        
        for attempt in range(max_retries):
            try:
                log(f"开始安装 {from_code}->{to_code} (尝试 {attempt + 1}/{max_retries})")
                ...
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(f"网络连接失败，请检查网络设置: {e}")
                ...
                log(f"找到 {len(available_packages)} 个可用包")
                ...
                if not target_package:
                    log(f"错误: 未找到 {from_code}->{to_code} 语言包")
                    # 显示可用包用于调试
                    for i, pkg in enumerate(available_packages[:5]):
                        if hasattr(pkg, 'from_code') and hasattr(pkg, 'to_code'):
                            log(f"  可用包: {pkg.from_code}->{pkg.to_code}")
                    return False
                
                log(f"找到目标包: {from_code}->{to_code}")
                if progress_callback:
                ...
                try:
                    with open(marker_path, 'w', encoding='utf-8') as f:
                        f.write(f"Installed via Windows API at {datetime.now()}\n")
                        f.write(f"Package: {from_code}_{to_code}\n")
                    log(f"标记文件已创建: {marker_path}")
                except OSError as e:
                    log(f"警告: 创建标记文件失败: {e}")
                ...
                log(f"✅ 成功安装 {from_code}->{to_code}")
                return True
                
            except Exception as e:
                log(f"安装失败 (尝试 {attempt + 1}/{max_retries}): {str(e)}")
                
                if attempt == max_retries - 1:
                    # 最后一次尝试失败
                    log(f"❌ 安装 {from_code}->{to_code} 完全失败")
                    
                    # 网络诊断
                    self._diagnose_network_windows()
                    
                    # 权限诊断
                    self._diagnose_permissions_windows()
    ...
    def _diagnose_network_windows(self):
        """Windows网络诊断"""
        try:
            import requests
            test_urls = [
            ...
            for url in test_urls:
                try:
                    ...
                    self.status_queue.put(f"网络测试 {url}: 正常 (状态码: {response.status_code})")
                    return  # 有一个成功就行
                except requests.exceptions.RequestException as e:
                    self.status_queue.put(f"网络测试 {url}: 失败 ({e})")
            
            self.status_queue.put("❌ 网络连接可能存在问题")
            self.status_queue.put("建议: 1) 检查防火墙设置 2) 检查代理配置 3) 重启网络连接")
            
        except Exception as e:
            self.status_queue.put(f"网络诊断失败: {e}")
    
    def _diagnose_permissions_windows(self):
        """Windows权限诊断"""
        test_file = os.path.join(self.package_dir, "permission_test.tmp")
        try:
            with open(test_file, 'w') as f:
                f.write("test")
            os.remove(test_file)
            self.status_queue.put("包目录权限: 正常")
        except OSError as e:
            self.status_queue.put(f"❌ 包目录权限错误: {e}")
            self.status_queue.put("建议: 以管理员身份运行程序")
    ...
    def install_package(self, from_code, to_code, progress_callback=None):
        ...
        if platform.system() == "Windows":
            self.status_queue.put(f"Windows系统使用Python API安装 {from_code}->{to_code}")
            return self._install_via_python_api_windows(from_code, to_code, progress_callback)
        else:
            # 非Windows系统使用原有逻辑
    ...
    def uninstall_package(self, from_code, to_code):
        ...
        if not self.is_package_installed(from_code, to_code):
            self.status_queue.put(f"语言包 {from_code}_{to_code} 未安装，无需卸载")
            return True
        ...
        try:
            if os.path.exists(marker_path):
                ...
                self.status_queue.put(f"✅ 已删除标记文件: {marker_file}")
            ...
            for package in installed_packages:
                if (hasattr(package, 'from_code') and hasattr(package, 'to_code') and
                    ...
                    if hasattr(package, 'uninstall'):
                        package.uninstall()
                        self.status_queue.put(f"✅ 通过API卸载成功: {from_code}->{to_code}")
                    break
            
            return True
            
        except Exception as e:
            self.status_queue.put(f"卸载失败: {e}")
            return False

    def diagnose_installation_environment(self):
        """诊断安装环境"""
        self.status_queue.put("=== Windows安装环境诊断 ===")
        self.status_queue.put(f"操作系统: {platform.system()} {platform.version()}")
        self.status_queue.put(f"Python版本: {sys.version}")
        ...
        # 检查必要库
        required_libs = ['argostranslate', 'requests', 'certifi']
        for lib in required_libs:
            try:
                __import__(lib)
                self.status_queue.put(f"{lib}: ✅ 已安装")
            except ImportError:
                self.status_queue.put(f"{lib}: ❌ 未安装")
        
        # 检查SSL配置
        ssl_cert_file = os.environ.get('SSL_CERT_FILE')
        self.status_queue.put(f"SSL_CERT_FILE: {ssl_cert_file}")
        if ssl_cert_file and os.path.exists(ssl_cert_file):
            self.status_queue.put("SSL证书: ✅ 正常")
        else:
            self.status_queue.put("SSL证书: ⚠️ 可能有问题")
        
        # 网络测试
        self._diagnose_network_windows()
        
        self.status_queue.put("=== 诊断完成 ===")
