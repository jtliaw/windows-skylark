...
class LanguagePackDialog(QDialog):
    """OCR语言包管理对话框 - Windows大小优化版本"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.main_window = parent
        self.setWindowTitle("OCR语言包管理器")
        self.setWindowIcon(QIcon("skylark.ico"))
    ...
    def setup_dialog_size(self):
        ...
        try:
            ...
            print(f"语言包管理器窗口设置为: {width}x{height}")
            ...
            if platform.system() == "Windows":
                ...
                self.setGeometry(200, 150, 530, 400)
...
class OCRLanguageTab(QWidget):
    ...
    def populate_lang_list(self):
        ...
        # 使用自定义目录的语言包列表
        all_installed_langs = custom_langs
        ...
        for code, name in SUPPORTED_LANGUAGES:
            ...
            # 获取语言包大小估计值
            size = PACKAGE_SIZE_ESTIMATES.get(code, 15)  # 默认15MB
...
class TranslatorOverlay(QWidget):
    ...
    def __init__(self, capture_rect, parent=None):
        super().__init__(None)  # 设置为顶级窗口，不设置父窗口
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            # 使用QTimer在主线程中执行翻译
            QTimer.singleShot(0, self.main_window.process_translation)
    ...
    def set_main_window(self, main_window):
        ...
        self.main_window = main_window
        if main_window:
            main_window.update_ui_signal.connect(self.handle_update_signal)
...
class ScreenTranslator(QMainWindow):
    ...
    def __init__(self):
        ...
        # 设置窗口属性
        self.setWindowTitle("Skylark Translation V3.0 - 在线版扫描屏幕翻译软件")
        try:
            # 尝试多个可能的图标路径
            possible_paths = [
                "skylark.ico",
                os.path.join(os.path.dirname(__file__), "skylark.ico"),
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "skylark.ico")
            ]
            
            if getattr(sys, 'frozen', False):
                possible_paths.insert(0, os.path.join(sys._MEIPASS, "skylark.ico"))
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    ...
                    if not icon.isNull():
                        ...
                        print(f"成功设置窗口图标: {path}")
                        icon_set = True
                        break
            
            if not icon_set:
                print("警告: 未能设置窗口图标，图标文件可能不存在或格式错误")
    ...
    def create_translator_overlay(self):
        ...
        if self.capture_area:
            ...
            # 创建独立的窗口
            self.translator_overlay = TranslatorOverlay(rect)
            # 设置主窗口引用
            self.translator_overlay.set_main_window(self)
            self.translator_overlay.show()
            self.overlay_hidden = False
    ...
    def setup_cross_platform_window_size(self):
        """设置跨平台兼容的窗口大小 - 针对Windows优化"""
        try:
            # 获取主屏幕的可用几何区域（减去任务栏等）
            screen = QApplication.primaryScreen()
            available_geometry = screen.availableGeometry()
            
            # 检测是否为Windows系统，使用更合适的默认大小
            system = platform.system()
            ...
            if system == "Windows":
                ...
                # 根据屏幕大小选择合适的窗口尺寸
                if screen_width >= 1920 and screen_height >= 1080:  # 1080p及以上
                    width = 800  # 增加宽度以适应高分辨率
                    height = 600  # 增加高度
                elif screen_width >= 1366 and screen_height >= 768:  # 768p
                    width = 650
                    height = 500
                else:  # 较小屏幕
                    width = 550
                ...
                dpi = screen.logicalDotsPerInch()
                if dpi > 120:  # 高DPI显示器
                    # 在高DPI下稍微减小窗口，因为控件会自动缩放
                    width = int(width * 0.9)
                    height = int(height * 0.9)
                    
            else:
                # 非Windows系统使用原有逻辑
                width = int(available_geometry.width() * 0.4)
                height = int(available_geometry.height() * 0.6)
            
            # 设置最小和最大限制
            width = max(500, min(width, 900))   # 增加最小宽度和最大宽度
            height = max(400, min(height, 700)) # 增加最小高度和最大高度
            ...
            self.setGeometry(x, y, width, height)
            self.setMinimumSize(500, 400)  # 增加最小尺寸
            self.setMaximumSize(900, 700)  # 增加最大尺寸
            
            print(f"Windows主窗口已设置为: {width}x{height} 位置: ({x}, {y})")
    ...
    def setup_windows_dpi_scaling(self):
        ...
        try:
            ...
            print(f"Windows DPI信息: DPI={dpi}, 设备像素比={device_ratio}")
            ...
            if scale_adjust != 1.0:
                ...
                new_width = int(current_geometry.width() * scale_adjust)
                new_height = int(current_geometry.height() * scale_adjust)
                
                # 确保不超出屏幕范围
                available_geometry = screen.availableGeometry()
                new_width = min(new_width, int(available_geometry.width() * 0.8))
                new_height = min(new_height, int(available_geometry.height() * 0.8))
                
                # 重新居中
                x = available_geometry.x() + (available_geometry.width() - new_width) // 2
                y = available_geometry.y() + (available_geometry.height() - new_height) // 2
                
                self.setGeometry(x, y, new_width, new_height)
                
                print(f"Windows DPI调整后窗口大小: {new_width}x{new_height}")
...
def main():
    ...
    if platform.system() == "Windows":
        ...
        try:
            ...
            # 尝试设置最新的DPI感知模式
            try:
                # Windows 10 版本1703及以上支持的Per-Monitor v2
                windll.user32.SetProcessDpiAwarenessContext(c_int(-4))
                print("Windows: 已设置Per-Monitor DPI Aware v2")
            except Exception:
                try:
                    # 回退到Per-Monitor DPI Aware
                    windll.shcore.SetProcessDpiAwareness(2)
                    print("Windows: 已设置Per-Monitor DPI Aware")
                except Exception:
                    # 最后回退到System DPI Aware
                    windll.user32.SetProcessDPIAware()
                    print("Windows: 已设置System DPI Aware")
                    
            # 设置缩放质量
            os.environ['QT_SCALE_FACTOR_ROUNDING_POLICY'] = 'RoundPreferFloor'
            
        except Exception as e:
            print(f"Windows DPI设置失败: {e}")
