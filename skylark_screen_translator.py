...
if not os.environ.get("ARGOS_PACKAGES_DIR"):
    ...
    argos_dir = os.path.join(app_dir, "argos_packages")
    os.makedirs(argos_dir, exist_ok=True)
    os.environ["ARGOS_PACKAGES_DIR"] = argos_dir

if not os.environ.get("TESSDATA_PREFIX"):
    app_dir = os.path.dirname(os.path.abspath(__file__))
    tess_dir = os.path.join(app_dir, "tessdata")
...
# 显示命令行依赖提示
def show_cli_dependency_prompt(missing_deps):
    """显示命令行依赖安装提示"""
    if missing_deps:
        print("警告：缺少以下系统依赖：")
...
def pre_launch_check():
    ...
    if missing:
        show_cli_dependency_prompt(missing)

# 如果是直接运行（不是被导入），则执行依赖检查
...
# 显示依赖安装提示
def show_dependency_prompt(missing_deps):
    """显示依赖安装提示"""
    # 延迟导入，避免在没有GUI环境的情况下出错
    try:
        ...
        msg.setWindowTitle("缺少系统依赖")
        
        if missing_deps:
            deps_text = ", ".join(missing_deps)
            msg.setText(f"缺少必要的系统依赖: {deps_text}")
            msg.setInformativeText("应用程序需要这些依赖才能正常运行。是否打开安装指南？")
        else:
            msg.setText("依赖检查完成")
            msg.setInformativeText("所有必要依赖已安装。")
        ...
        if result == QMessageBox.Yes and missing_deps:
            # 打开安装指南
            webbrowser.open("https://github.com/jtliaw/Skylark-Screen-Translator/wiki/Linux-Installation")
        ...
        if missing_deps:
            ...
            for dep in missing_deps:
                print(f"  - {dep}")
            print("请参考安装指南: https://github.com/jtliaw/Skylark-Screen-Translator/wiki/Linux-Installation")
            print("按回车键继续启动，或按Ctrl+C退出")
            input()
...
if __name__ != "__main__":
    ...
    missing = check_dependencies()
    if missing:
        show_dependency_prompt(missing)
