...
class OCRLanguageTab(QWidget):
    ...
    def setup_ui(self):
        ...
        # 设置尺寸策略和最小尺寸
        self.install_btn.setSizePolicy(QSizePolicy.MinimumExpanding, QSizePolicy.Preferred)
        self.install_btn.setMinimumSize(100, 30)
        self.install_btn.clicked.connect(lambda: self.install_ocr_language())
        btn_layout.addWidget(self.install_btn)
        
        self.remove_btn = QPushButton("删除选中语言包")
        self.remove_btn.setSizePolicy(QSizePolicy.MinimumExpanding, QSizePolicy.Preferred)
        self.remove_btn.setMinimumSize(100, 30)
        self.remove_btn.clicked.connect(self.remove_ocr_lang)
        btn_layout.addWidget(self.remove_btn)
        
        self.refresh_btn = QPushButton("刷新列表")
        self.refresh_btn.setSizePolicy(QSizePolicy.MinimumExpanding, QSizePolicy.Preferred)
    ...
    def __init__(self):
        ...
        if platform.system() == "Windows":
            ...
            try:
                ...
                # 测试写入权限
                test_file = os.path.join(tessdata_dir, "test_write.tmp")
                with open(test_file, 'w') as f:
                    f.write("test")
                os.remove(test_file)
                ...
                # 如果可写，使用程序目录
                os.environ['TESSDATA_PREFIX'] = tessdata_dir
                print(f"Windows: 使用程序目录存储OCR语言包: {tessdata_dir}")
                ...
                print(f"Windows: 程序目录不可写({e})，使用用户目录: {user_tessdata_dir}")
