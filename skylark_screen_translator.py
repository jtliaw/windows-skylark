...
def setup_custom_package_dir(custom_dir):
    """
    设置自定义包目录，并确保 Argos Translate 使用它 - Windows 专用修复
    """
    # Windows 系统直接设置环境变量，不创建符号链接
    if platform.system() == "Windows":
        ...
        print(f"Windows: 已设置环境变量 ARGOS_PACKAGES_DIR = {custom_path}")
        return custom_path
    
    # 原有非 Windows 系统的代码保持不变
    custom_path = Path(custom_dir)
...
class Translator:
    ...
    def initialize(self):
        ...
        try:
            ...
            # 记录当前环境变量
            self.log(f"当前 ARGOS_PACKAGES_DIR: {os.environ.get('ARGOS_PACKAGES_DIR', '未设置')}")
            
            # Windows 专用检查
            if platform.system() == "Windows":
                package_dir = os.environ.get('ARGOS_PACKAGES_DIR')
                if package_dir and os.path.exists(package_dir):
                    self.log(f"Windows: 检查包目录内容: {os.listdir(package_dir)}")
            ...
            if 'ARGOS_PACKAGES_DIR' in os.environ:
                ...
                # Windows 上直接设置环境变量，不尝试调用可能不存在的 API
                if platform.system() != "Windows":
                    try:
            ...
            if not installed_languages:
                ...
                # Windows 专用提示
                if platform.system() == "Windows":
                    self.log("Windows 提示: 请确保语言包文件 (.argosmodel) 已放置在正确目录")
                    package_dir = os.environ.get('ARGOS_PACKAGES_DIR', '未知')
                    self.log(f"包目录应该是: {package_dir}")
...
class PackageManager:
    ...
    def diagnose_windows_issues(self, from_code, to_code):
        """Windows 专用问题诊断"""
        self.status_queue.put("=== Windows 专用诊断 ===")
        
        # 检查包目录
        package_dir = self.package_dir
        self.status_queue.put(f"包目录: {package_dir}")
        self.status_queue.put(f"目录存在: {os.path.exists(package_dir)}")
        self.status_queue.put(f"目录可写: {os.access(package_dir, os.W_OK)}")
        
        # 列出包目录中的所有文件
        if os.path.exists(package_dir):
            files = os.listdir(package_dir)
            self.status_queue.put(f"目录中的文件: {len(files)} 个")
            for file in files:
                file_path = os.path.join(package_dir, file)
                is_dir = os.path.isdir(file_path)
                size = os.path.getsize(file_path) if not is_dir else 0
                self.status_queue.put(f"  - {file} ({'目录' if is_dir else f'{size} 字节'})")
        
        # 检查特定语言包文件
        package_file = f"{from_code}_{to_code}.argosmodel"
        package_path = os.path.join(package_dir, package_file)
        self.status_queue.put(f"语言包文件: {package_path}")
        self.status_queue.put(f"文件存在: {os.path.exists(package_path)}")
        
        if os.path.exists(package_path):
            self.status_queue.put(f"文件大小: {os.path.getsize(package_path)} 字节")
        
        # 检查环境变量
        self.status_queue.put(f"ARGOS_PACKAGES_DIR: {os.environ.get('ARGOS_PACKAGES_DIR', '未设置')}")
        
        self.status_queue.put("=== Windows 诊断结束 ===")

    def _get_package_dir(self):
        """
        获取语言包存储目录，使用动态检测的目录 - Windows 专用修复
        """
        # 确定应用程序目录
        ...
        # Windows 专用路径设置
        if platform.system() == "Windows":
            # 在 Windows 上使用用户目录而不是程序目录
            windows_dir = Path.home() / "AppData" / "Local" / "SkylarkTranslator" / "argos_packages"
            windows_dir.mkdir(parents=True, exist_ok=True)
            
            # 设置环境变量
            os.environ['ARGOS_PACKAGES_DIR'] = str(windows_dir)
            
            self.status_queue.put(f"Windows: 使用用户目录存储语言包: {windows_dir}")
    ...
    def _install_via_python_api(self, from_code, to_code, progress_callback=None):
        ...
        def log(message):
            self.status_queue.put(f"[Python API] {message}")
            print(f"[Python API] {message}")
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                log(f"开始安装 {from_code}->{to_code} (尝试 {attempt}/{max_retries})")
                if progress_callback:
                    progress_callback(10)
    
                # 1. 设置包目录环境变量（确保安装到正确位置）
                ...
                try:
                    ...
                    log(f"更新包索引网络错误: {e}. 将在 {retry_delay} 秒后重试...")
                    if attempt < max_retries:
                        time.sleep(retry_delay)
                        continue # 重试循环
                    else:
                        raise Exception(f"更新包索引失败 after {max_retries} 次尝试: {e}")
                except Exception as e:
                    log(f"更新包索引失败: {e}")
                    if "SSL" in str(e).upper():
                        log("SSL错误，尝试使用 certifi 的 CA 包...")
                        # 修复AppImage网络问题 (确保certifi可用)
                        os.environ['SSL_CERT_FILE'] = certifi.where()
                        os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
                        if attempt < max_retries:
                            time.sleep(retry_delay)
                            continue
                    raise e # 如果不是SSL或重试用完，重新抛出异常
                ...
                if not target_package:
                    error_msg = f"错误: 未找到 {from_code}->{to_code} 语言包"
                    log(error_msg)
                    # 列出所有可用包用于调试
                    for i, pkg in enumerate(available_packages[:10]):  # 只显示前10个避免信息过多
                        if hasattr(pkg, 'from_code') and hasattr(pkg, 'to_code'):
                            log(f"  可用包 {i+1}: {pkg.from_code}->{pkg.to_code}")
                        elif hasattr(pkg, 'package_code'):
                            log(f"  可用包 {i+1}: {pkg.package_code}")
                    # 检查这个包是否在官方列表中
                    if not self.is_package_available(from_code, to_code):
                        log(f"额外提示: {from_code}->{to_code} 不在已知的官方包列表中。")
                    return False # 不是网络错误，重试没用
                ...
                try:
                    ...
                    log(f"下载包网络错误: {e}. 将在 {retry_delay} 秒后重试...")
                    if attempt < max_retries:
                        time.sleep(retry_delay)
                        continue # 重试循环
                    else:
                        raise Exception(f"下载包失败 after {max_retries} 次尝试: {e}")
                ...
                try:
                    ...
                    log(f"警告: 无法创建标记文件 {marker_path}: {e}. 但包可能已安装。")
                ...
                log(f"安装失败 (尝试 {attempt}/{max_retries}): {str(e)}")
                log(f"详细错误:\n{error_detail}")
    
                if attempt == max_retries:
                    # 最后一次尝试也失败了
                    log(f"❌ 安装 {from_code}->{to_code} 彻底失败 after {max_retries} 次尝试。")
                    ...
                    try:
                        ...
                        log(f"网络连接测试: 失败 ({net_err})")
                        log("请检查您的互联网连接。")
                    except Exception as net_err:
                        log(f"网络连接测试: 发生意外错误 ({net_err})")
    
                    # 检查包目录权限
                    test_file_path = os.path.join(self.package_dir, "write_test.tmp")
                    try:
                        ...
                        log(f"包目录写入权限: 失败 ({perm_err})")
                        log(f"请确保程序有权限写入目录: {self.package_dir}")
                        log("尝试以管理员身份运行程序可能解决此问题。")
...
class ScreenTranslator(QMainWindow):
    ...
    def __init__(self):
        ...
        if platform.system() == "Windows":
            # 在 Windows 上使用用户目录而不是程序目录
            argos_package_dir = Path.home() / "AppData" / "Local" / "SkylarkTranslator" / "argos_packages"
            argos_package_dir.mkdir(parents=True, exist_ok=True)
            
            # 直接设置环境变量
            os.environ['ARGOS_PACKAGES_DIR'] = str(argos_package_dir.absolute())
            
            # 设置 Tesseract tessdata 目录
            tessdata_dir = argos_package_dir / "tessdata"
            tessdata_dir.mkdir(parents=True, exist_ok=True)
            os.environ['TESSDATA_PREFIX'] = str(tessdata_dir.absolute())
            
            print(f"Windows: 设置 Argos 包目录: {argos_package_dir.absolute()}")
            print(f"Windows: 设置 Tesseract 数据目录: {tessdata_dir.absolute()}")
        else:
            # 非 Windows 系统使用原有逻辑
