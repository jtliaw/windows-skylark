...
class PackageManager:
    ...
    def _install_via_python_api(self, from_code, to_code, progress_callback=None):
        """通过Python API安装语言包（Windows专用增强版）"""
        import requests
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Windows特定设置 - 确保使用正确的包目录
                windows_package_dir = self.package_dir
                
                # 确保目录存在且有写入权限
                try:
                    os.makedirs(windows_package_dir, exist_ok=True)
                    # 测试写入权限
                    test_file = os.path.join(windows_package_dir, "write_test.tmp")
                    with open(test_file, 'w') as f:
                        f.write("test")
                    os.remove(test_file)
                    log(f"Windows包目录可写: {windows_package_dir}")
                except PermissionError:
                    # 如果无权限，尝试使用用户目录
                    user_dir = os.path.join(os.path.expanduser("~"), "argos_packages")
                    os.makedirs(user_dir, exist_ok=True)
                    windows_package_dir = user_dir
                    log(f"切换到用户目录: {windows_package_dir}")
                except Exception as e:
                    log(f"目录访问错误: {e}")
                    return False
    
                # 设置包目录环境变量
                os.environ['ARGOS_PACKAGES_DIR'] = windows_package_dir
                log(f"设置包目录: {windows_package_dir}")
    
                # 对于Windows，确保包目录被正确设置
                try:
                    if hasattr(argostranslate.package, 'set_packages_dir'):
                        argostranslate.package.set_packages_dir(windows_package_dir)
                        log(f"已设置包目录API: {windows_package_dir}")
                except Exception as e:
                    log(f"设置包目录API失败: {e}")
                ...
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        # 检查是否是代理问题
                        if "proxy" in str(e).lower() or "407" in str(e):
                            log("检测到可能的代理问题，请检查网络设置")
                ...
                # 7. 创建标记文件
                marker_file = f"{from_code}_{to_code}.argosmodel"
                marker_path = os.path.join(windows_package_dir, marker_file)
                try:
    ...
    def _windows_specific_fixes(self):
        """Windows特定的修复措施"""
        # 确保使用正确的路径分隔符
        self.package_dir = os.path.normpath(self.package_dir)
        ...
        # 检查并修复Windows路径问题
        if not os.access(self.package_dir, os.W_OK):
            # 尝试使用用户目录作为备选
            user_dir = os.path.join(os.path.expanduser("~"), "argos_packages")
            os.makedirs(user_dir, exist_ok=True)
            if os.access(user_dir, os.W_OK):
                self.package_dir = user_dir
                os.environ['ARGOS_PACKAGES_DIR'] = self.package_dir
                self.status_queue.put(f"切换到用户目录: {self.package_dir}")
